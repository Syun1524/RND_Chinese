...
| 补丁体积 | 356 MB | **安装包 223.8 MB**（补丁包本体 932.6 MB，含 735 MB 字形种子） |
...
7. **数字 `9` 不显示**、**`json type_error.302` 崩溃**、**缺字形抛异常**。详见上表。
...
11. **引号「」占满一个全角格、换行后对不齐**。引号的墨只占格子右半边，导致
    ...
    墨贴左缘。
12. **中文没有空格导致长句不换行**。改为 CJK 逐字断行 + 行首/行尾禁则（标点不排行首）。
...
> ⚠️ 第一版验证写的是「提取 → 回写 → 与原文件逐字节比对」，结果**满屏假绿**。
> 原因有两个：`replace-text` 按文件名 stem 配对，脚本叫 `f.msb` 而文本叫
> `f.msb.txt` 时配不上，**静默跳过、退出码 0**；提取文本经归一化，回写时本就
> 会重写若干行，**「逐字节往返」根本不是正确判据**（旧工具同样如此）。
