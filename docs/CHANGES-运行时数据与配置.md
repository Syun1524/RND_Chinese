...
## 2026-09-12 撤下已废弃的图（含 c0data 索引重排）

### 背景

作者把不要的图放在 `临时/cn/汉化好的/bg/废弃/`。核对发现其中 **10 个已经上线**
在补丁包里（曾被 `fileRedirection` 重定向）—— 既然废弃，就不该继续发。

### 撤下的条目

| 归档/fileId | c0data 名 |
|---|---|
| bg/252 | `rnd_ibg058b.png` |
| bg/285 | `rnd_ibg083a.png` |
| bg/420..427 | `movie_dar005b..h_last.png`、`movie_dar013_last.png` |

（`rnd_ibg058a` 在废弃目录里也有一份，但顶层有同内容的 `重做` 版 → **保留**。）

### 关键：必须重排 `c0data.cls`

`fileRedirection` 存的是 **c0data 数组下标**。删掉 10 个数组元素后，
若不同步重排下标，**其后的每一条重定向都会指错文件**（本项目出过事故：
标题 UI 整个空白，索引指到了角色模型）。

做法（`scripts/drop_deprecated.py`）：
1. 从 `c0data.cls` 保序移除这 10 行；
2. 用「旧名字 → 新下标」重写 `base.fileRedirection` 里所有下标；
3. 自检：每条重定向的新下标所指名字，必须与旧下标所指名字相同。

`c0data.cls` 88 → 78 行，`c0data/` 88 → 78 文件。

### 归档

撤下的图 + 其余从未上线的废弃图，一起放进 `成品ing/补丁包/废弃图片/`（24 个，
带 `README.md` 说明来龙去脉）。该目录被 `build_installer.py` 与
`gen_pkg_manifest.py` 排除，**不进玩家安装包**。

内容重复的不重复存放（曾因「原件」与「撤下的 c0data 文件」同名不同而
产生两份，已去重）。

### 验证

- 解包安装包：`c0data` 78 个、`enscript` 322 个，撤下的 10 个不残留，`废弃图片/` 不在包内。
- `final_accept.py` 全流程通过（安装 6 秒 → log.txt 5385 B / 0 错误 → 卸载 16 秒 → 回纯净）。
  撤索引后游戏仍正常加载，说明重排正确。
- 安装包 154.0 → **142.4 MB**（少了 10 张废弃图，其中 8 张是 movie 末帧）。
